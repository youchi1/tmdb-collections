...
async function generateCatalogRequests(config) {
  ...
  for (const catalog of catalogs.slice().reverse()) {
    //reverse the catalogs to buffer topRatedByCountry and topRatedByCompany first, as they are using sqlite cache so the same collections from other in-memory catalogs will not be cached as in-memory
