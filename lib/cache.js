...
function initCache() {
  ...
  try {
    ...
    const cachePath = path.join(sqliteDir, "cache.sqlite");

    // Check sqlite directory
    if (!fs.existsSync(sqliteDir)) {
      console.log("[cache.js] ❌ SQLite directory not found at", sqliteDir);
    } else if (fs.existsSync(cachePath)) {
      const stats = fs.statSync(cachePath);
      console.log(`[cache.js] 📊 SQLite cache size: ${stats.size} bytes (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
    } else {
      console.log("[cache.js] ❌ cache.sqlite not found at", cachePath);
    }
  } catch (e) {
    console.log("[cache.js] ❌ Could not determine cache.sqlite size:", e);
  }
