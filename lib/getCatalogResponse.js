...
async function getCatalogResponse(req) {
  ...
  const catalogId = req.params.id.split(".")[1];
  ...
  const genre = manifest.catalogs.find((c) => c.id.includes(catalogId))?.extra[0]?.genres.find((g) => g.name === extra.genre);
  extra.genreId = genre?.id;

  const cacheKey = JSON.stringify({
    id: catalogId, //catalog
